...
std::tuple<int, int, std::unique_ptr<int[]>, std::unique_ptr<char[]>,
  std::unique_ptr<float[]>, std::unique_ptr<float[]>> loadSPP(
    std::string fname,
    float phiInit) {
  ...
  try {
    if(f.is_open()) {
      ...
      U = std::make_unique<float[]>(n), phi = std::make_unique<float[]>(n); 
      for(i = 0; i < n*m; i++)
  ...
  return std::make_tuple(m, n, std::move(C), std::move(A), std::move(U), std::move(phi));
}
