...
int main() {
    ...
    for(auto instance : fnames) {
            ...
            for(run = 0; run < NUM_RUN; run++) {
                ...
                TIMED(t,
                    ReactiveGRASP(m, n, C, A, U, zInits, zAmels, zBests, phi, PHI_OFFSET,
                        alpha, pAlpha, _NBU_, DELTA, NUM_ITER, DEEPSEARCH, PARALLEL);
                    zGRASP = zBests[NUM_ITER-1];
                    done_iter = ACO(m, n, C, A, U, zGRASP, zInits, zAmels, zBests,
                        pExploit, phi, &phi_bef, &phi_aft, PHI_NUL, RHO_E,
